 ...
 *
 * Centralized service for managing navigation state across the enterprise app.
 * Features:
 * - Detects active app context from routes
 * - Lazy-loads feature manifests with caching
...
/**
 * Local feature manifest loaders (direct imports instead of module federation)
 */
const MANIFEST_LOADERS: Record<string, () => Promise<{ remoteManifest: NavigationManifest }>> = {
  finance: () => import('../../features/finance/manifest'),
  hr: () => import('../../features/hr/manifest'),
  srm: () => import('../../features/srm/manifest'),
  pm: () => import('../../features/pm/manifest'),
  warehouses: () => import('../../features/warehouses/manifest'),
};
...
export class NavigationFacadeService {
  ...
  private detectAndLoadManifest(url: string): void {
    ...
    this.loadFeatureManifest(appId);
  }
  ...
  private extractAppIdFromUrl(url: string): string {
    ...
    // Default to shell
    this.log('No feature app detected, using shell', { url });
    return 'shell';
  }

  /**
   * Load manifest from a local feature module with caching
   */
  private async loadFeatureManifest(appId: string): Promise<void> {
    const loader = MANIFEST_LOADERS[appId];
    if (!loader) {
      this.log('No manifest loader found for appId:', appId);
      this.setShellManifest();
      return;
    }
    ...
    this.log('Loading manifest for feature:', appId);
    ...
    try {
      ...
      this.log('Failed to load manifest for:', appId);

      // Get display name from registry
      const registry = REMOTE_REGISTRY[appId];
      const displayName = registry?.displayName || appId;
  ...
  reloadManifest(): void {
    ...
    if (currentAppId && currentAppId !== 'shell') {
      ...
      this.loadFeatureManifest(currentAppId);
    }
  }
  ...
  private log(message: string, data?: unknown): void {
    ...
    console.log(`[NavigationFacade]`, message, data ?? '');
  }
  ...
  private updateActiveModuleFromUrl(url: string): void {
    ...
    this.permissionsFacade.ensureModulePermissions(moduleKey).catch((error) => {
      this.log('Failed to load permissions for module', {
        moduleKey,
